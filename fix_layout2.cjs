...
// The replacement metrics block for the header
const headerMetricsBlock = `        <div className="flex flex-wrap items-center gap-3 lg:gap-5 shrink-0 bg-white border border-slate-200/50 py-1.5 px-3 rounded-lg shadow-sm ml-auto mr-4">
...
if (!content.includes('<Flame className="w-3.5 h-3.5 text-rose-500" /> Incursions')) {
  content = content.replace('        <div className="flex items-center gap-2 shrink-0 ml-auto">', headerMetricsBlock);
}

// 2. Add the big table below the map
const tableString = `      </div>
...
if (!content.includes('id="tightly-packed-threat-table"')) {
    content = content.replace(/      <\/div>\s*<\/div>\s*\);\s*\}\s*export default WorldThreatMapView;/, tableString);
}

// Ensure the floating metrics in the map are removed
const floatingMetricsStart = '{/* Top-Right: Tactical Metrics */}';
const floatingMetricsEnd = '                </div>\n              </div>\n            </div>';
if (content.includes(floatingMetricsStart)) {
  const startIdx = content.indexOf(floatingMetricsStart);
  let endIdx = content.indexOf(floatingMetricsEnd, startIdx) + floatingMetricsEnd.length;
